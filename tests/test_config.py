...
def test_print_topics_only_parsed_from_scraping_block(tmp_path):
    settings = load_settings(
        _write_config(tmp_path, "  print_topics_only: true"), env_file=None
    )
    assert settings.scraping.print_topics_only is True


def test_print_topics_only_defaults_to_false(tmp_path):
    settings = load_settings(_write_config(tmp_path, "  retries: 3"), env_file=None)
    assert settings.scraping.print_topics_only is False


def test_resolve_print_topics_only_inherits_global_when_absent():
    assert resolve_print_topics_only(True, {}) is True
    assert resolve_print_topics_only(False, {"per_page": 100}) is False


def test_resolve_print_topics_only_source_override_wins():
    assert resolve_print_topics_only(True, {"print_topics_only": False}) is False
    assert resolve_print_topics_only(False, {"print_topics_only": True}) is True
