...
from manufacturer_scraper.models import Article, is_print_related, normalize_url, utcnow
...
class SourceResult:
    ...
    skipped_offtopic: int = 0  # not print / document-imaging related
    pushed: int = 0
    failed: int = 0
    dry_run: bool = False
...
def run_source(
    ...
    *,
    ...
    - print_topics_only: skip articles that don't look print / document-
      imaging related (keyword match on title + categories + summary). Used to
      drop non-print news (laptops, financials, healthcare) from the big
      diversified vendors; leave off for specialist print/document sources.
    """
    ...
    for raw in source.iter_articles(max_pages=max_pages, is_seen=is_seen):
        ...
        if print_topics_only and not is_print_related(
            article.title, article.categories, article.summary
        ):
            result.skipped_offtopic += 1
            log.debug("OFFTOPIC %s | %s", source.name, article.title)
            continue
...
def print_summary(results: list[SourceResult]) -> None:
    header = (
        f"{'source':<22}{'found':>7}{'new':>6}{'seen':>6}{'old':>6}{'topic':>6}"
    ...
    for r in results:
        print(
            ...
            f"{r.skipped_offtopic:>6}{r.pushed:>8}{r.failed:>8}{r.duration_s:>7.1f}s"
        )
    totals = SourceResult(
        source="TOTAL",
        ...
        skipped_offtopic=sum(r.skipped_offtopic for r in results),
    ...
    print(
        ...
        f"{totals.skipped_old:>6}{totals.skipped_offtopic:>6}{totals.pushed:>8}"
        f"{totals.failed:>8}{totals.duration_s:>7.1f}s"
    )
