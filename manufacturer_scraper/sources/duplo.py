"""Duplo USA newsroom — WordPress REST API (duplousa.com)."""
...
API_BASE = "https://www.duplousa.com/wp-json/wp/v2"


def parse_duplo_categories(payload: list[dict]) -> dict[int, str]:
    return {int(cat["id"]): strip_html(cat.get("name", "")) for cat in payload}


def parse_duplo_posts(payload: list[dict], cat_map: dict[int, str]) -> list[Article]:
...
class DuploSource(BaseSource):
    ...
    def iter_articles(
        ...
        *,
        ...
        cat_map = parse_duplo_categories(cat_response.json())
        ...
        while max_pages is None or page <= max_pages:
            ...
            batch = parse_duplo_posts(response.json(), cat_map)
            if not batch:
                break
            yield from batch
            if all(is_seen(article.url) for article in batch):
                log.debug("duplo: page %d fully seen — stopping", page)
                break
            if page >= total_pages:
                break
            page += 1
